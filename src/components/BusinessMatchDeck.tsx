...
import type { Business, Enquiry } from '../data/mock';
...
export type BusinessMatchItem =
  | { kind: 'business'; id: string; business: Business }
  | { kind: 'enquiry'; id: string; enquiry: Enquiry };

type Decision = 'pass' | 'interested';

export function BusinessMatchDeck({
  items,
  creditsUsed,
  creditLimit = 10,
  city,
  ...
  const currentItem = items[cardIndex % Math.max(items.length, 1)];
  const nextItem = items[(cardIndex + 1) % Math.max(items.length, 1)];
  ...
  const commit = (decision: Decision) => {
    ...
    Animated.timing(position, {
      ...
      onDecision(decision, currentItem);
  ...
  const panResponder = useMemo(
    ...
    [creditLimit, creditsUsed, currentItem, locked, onDecision, position, screenWidth],
  );
  ...
  if (!currentItem) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', padding: spacing[5] }}>
        <Text variant="heading3">No business matches available yet.</Text>
        <Text variant="body" tone="secondary" style={{ marginTop: spacing[2] }}>
          New businesses and enquiries that fit your profile will appear here.
        </Text>
      </View>
    );
  }
    ...
    >
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: spacing[3] }}>
        <View style={{ flex: 1 }}>
          ...
          <Text variant="heading2">Business matches</Text>
        </View>
        <View style={{ alignItems: 'flex-end', gap: spacing[1] }}>
          <Chip label={`Near ${city}`} selected />
          <Text variant="bodySmall" tone={locked ? 'warning' : 'tertiary'}>
            {Math.max(creditLimit - creditsUsed, 0)} swipes left
          </Text>
        </View>
      </View>
      ...
      <View style={{ flex: 1, minHeight: 0, paddingBottom: spacing[3] }}>
        {nextItem ? (
          <View style={{ position: 'absolute', inset: 8, top: 14, opacity: 0.55 }}>
            <BusinessMatchCard item={nextItem} compact={compact} />
          </View>
        ) : null}
        <Animated.View
            ...
            actions={
              <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: spacing[8] }}>
                ...
                <DecisionButton
                  kind="interested"
                  locked={locked}
                  onPress={() => (locked ? setPremiumOpen(true) : commit('interested'))}
                />
              </View>
            }
          />
...
function BusinessMatchCard({
  ...
  item: BusinessMatchItem;
  compact: boolean;
  actions?: React.ReactNode;
  onOpen?: () => void;
}) {
  const isBusiness = item.kind === 'business';
  const business = isBusiness ? item.business : undefined;
  const enquiry = !isBusiness ? item.enquiry : undefined;
  const reasons = business?.whyItMatches ?? enquiry?.whyItFits ?? [];
    ...
    >
      <View style={{ gap: spacing[2] }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: spacing[3] }}>
          <Text variant="micro" tone="chrome">
            {isBusiness ? 'Matches what you need' : 'Needs what you offer'}
          </Text>
          {(business?.match ?? enquiry?.match) ? <MatchLabel quality={(business?.match ?? enquiry?.match)!} /> : null}
        </View>

        {business ? (
          <>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing[3], marginTop: spacing[2] }}>
              <Logo name={business.name} size={compact ? 'md' : 'lg'} />
              <View style={{ flex: 1, gap: spacing[1] }}>
                <Text variant="labelLarge">{business.role}</Text>
                {business.trust ? <TrustBadge signal={business.trust} detail="business" /> : null}
              </View>
            </View>
            <Text variant="heading1">{business.name}</Text>
            <Text variant="body" tone="secondary">{business.city}, {business.region}</Text>
            <Text variant="heading3" tone="chrome">{business.capability}</Text>
          </>
        ) : enquiry ? (
          <>
            <Text variant="heading1" style={{ marginTop: spacing[2] }}>{enquiry.title}</Text>
            <Text variant="heading3" tone="chrome">{enquiry.budget}</Text>
            <Text variant="body" tone="secondary">{enquiry.location} · Needed by {enquiry.neededBy}</Text>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing[2] }}>
              <Text variant="labelLarge">{enquiry.buyer}</Text>
      ...
      {!compact && business ? (
        <View style={{ marginTop: spacing[6], gap: spacing[2] }}>
          <Text variant="micro" tone="tertiary">About</Text>
          <Text variant="body" tone="secondary" numberOfLines={3}>{business.about}</Text>
        </View>
      ) : null}
      ...
      <View style={{ marginTop: 'auto', gap: spacing[3], paddingTop: spacing[3] }}>
        {onOpen ? (
          <Button
            label={isBusiness ? 'View business profile' : 'View full enquiry'}
            variant="secondary"
            onPress={onOpen}
          />
        ) : null}
...
function DecisionButton({ kind, locked = false, onPress }: { kind: Decision; locked?: boolean; onPress: () => void }) {
        ...
        >
          <View style={{ gap: spacing[2] }}>
            ...
            <Text variant="body" tone="secondary">
              Upgrade to keep exploring matched businesses and enquiries.
            </Text>
          </View>
